...
import { NewBillOfLadingHub } from './newBillOfLadingHub'
...
const DownloadCfdiListCsvButton = ({ dateRange, reportType, label, tooltip, BoLHClients, BoLHUsers }) => {
  const downloadCsv = () => {
    const url = `/api/bill-of-lading-hub/list-csv?from=${dateRange[0].toISOString()}&to=${dateRange[1].toISOString()}&BoLHCients=${BoLHClients}&BoLHUsers=${BoLHUsers}&reportType=${reportType}`
    post(url)
...
const BillOfLadingPageHeader = ({
  ...
  const [isNewBillOfLadingHubVisible, setIsNewBillOfLadingHubVisible] = useState(false)
      ...
      <Col>
        <Button
          ...
          onClick={() => {
            ...
            setIsNewBillOfLadingHubVisible(true)
          }}
  ...
  return (
    <PageHeader ghost={false} title={i18n('billOfLading.title')} subTitle={<CfdiTypeFilter />} extra={extra} className="text-right py-1">
      ...
      <Drawer
        width="80%"
        onClose={() => setIsNewBillOfLadingHubVisible(false)}
        visible={isNewBillOfLadingHubVisible}
        bodyStyle={{ paddingBottom: 80 }}
        destroyOnClose
        // eslint-disable-next-line prettier/prettier
        // extra={
        //   <Space>
        //     <Button onClick={() => setIsNewBillOfLadingHubVisible(false)}>Cancel</Button>
        //     {/* <Button onClick={() => setIsNewBillOfLadingHubVisible(false)} type="primary">
        //       {i18n('buttons.submit')}
        //     </Button> */}
        //   </Space>
        //   // eslint-disable-next-line prettier/prettier
      ...
      >
        <NewBillOfLadingHub
          setIsNewBillOfLadingHubVisible={setIsNewBillOfLadingHubVisible}
  ...
  const menu = (
    <Menu>
      ...
      {cfdi.cfdiType === 'invoice' && !cfdi.canceledAt && moment(cfdi.createdAt).isSame(new Date(), 'month') ? (
        <Menu.Item
          icon={cancelInProgress ? <Spin /> : <CloseCircleTwoTone twoToneColor="#f97474" />}
      ...
      <List size="small" bordered={false}>
        <List.Item>
          <div className="w-full">
            <a href={`bill-of-lading-hub/${BoLHub._id}`} className="text-xs">
              {BoLHub.folio}
            </a>
            <br />
            <a href={`bill-of-lading-hub/${BoLHub._id}`} className="text-xl">
              {BoLHub.name}
            </a>
...
const CopyHubModal = ({ visible, hubIds, setVisible, originalHubs, hubCounter }) => {
  ...
  // post('/api/bill-of-lading-hub/get-hubs-and-folio-for-hub-copy', { body: { hubIds }}).then(({ originalHubs, hubCounter }) => {
...
const BoLHList = ({ dateRange, BoLHType, BoLHStatus, BoLHClients, BoLHUsers }) => {
  ...
  const listUrl = `/api/bill-of-lading-hub/list?from=${dateRange[0].toISOString()}&to=${dateRange[1].toISOString()}&BoLHClients=${BoLHClients}&BoLHUsers=${BoLHUsers}`
  ...
  const copyHub = id => {
    post(`/api/bill-of-lading-hub/copy-single-hub?id=${id}`)
  ...
  const copyMultipleHub = hubIds => {
    ...
    const { result, insertedIds } = post(`/api/bill-of-lading-hub/copy-multiple-hubs`, { body: hubIds })
      // eslint-disable-next-line
      .catch(error => console.log(error))
  ...
  const columns = [
      ...
      render: (text, record) => (
        <div>
          <Paragraph className="my-0 text-md font-medium">
            <a href={`/bill-of-lading-hub/${record._id}`}>{record.name || record.folio}</a>
          </Paragraph>
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          <Paragraph className="my-0 text-md font-semibold">{record.originPlace ? record.originPlace : null}</Paragraph>
          <Paragraph className="my-1 text-xs">{record.originCompany ? record.originCompany : null }</Paragraph>
          <Paragraph className="my-1 text-xs">{record.originDate ? record.originDate : null}</Paragraph>
        </div>
      ),
      responsive: ['sm'],
    },
  ...
  const dataForTable = data.BoLHubs.map(BoLHub => {
    ...
    const origins = BoLHub?.billOfLading2?.locations.map(location => {
      if (location?.locationType?.print === 'Origen')
        return { company: location?.company?.name ? location?.company?.name : null, date: location?.departureArrivalDateTime?.print ? location?.departureArrivalDateTime?.print : null, place: location?.place?.name ? location?.place?.name : null }
      return null
    })
    const destinations = BoLHub?.billOfLading2?.locations.map(location => {
  ...
  return (
    ...
    <div>
      <Space className="mt-4">
        <Dropdown overlay={actionsMenuForMultipleHubs(selectedHubIds)} placement="bottomLeft">
      ...
      <Table rowSelection={rowSelection} columns={columns} dataSource={dataForTable} className="mt-4" bordered={false} pagination={false} />
      <CopyHubModal
...
export const BillOfLadingPage = () => {
  const defaultDateRange = [moment().subtract(30,"days"), moment()] // Before: [moment().startOf('month'), moment().endOf('month')]
      ...
      <Layout>
        ...
        <Layout.Content className="p-4">
          <BillOfLadingPageHeader
            ...
            BoLHClients={BoLHClients}
          />
          <BoLHList dateRange={dateRange} BoLHClients={BoLHClients} BoLHUsers={BoLHUsers} />
        </Layout.Content>
      </Layout>
