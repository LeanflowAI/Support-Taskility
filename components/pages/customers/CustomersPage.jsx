...
const CustomersPageHeader = ({
  ...
  return (
    <PageHeader ghost={false} title={i18n('customers.title')} subTitle={<CfdiTypeFilter />} extra={extra} className="text-right py-1">
      ...
      >
        <NewCustomer
          setIsNewCustomerVisible={setIsNewCustomerVisible}
...
const CustomersList = ({ dateRange, BoLHType, BoLHStatus, BoLHClients, BoLHUsers }) => {
  ...
  const listUrl = `/api/customers/get-all-customers` // ?from=${dateRange[0].toISOString()}&to=${dateRange[1].toISOString()}&BoLHClients=${BoLHClients}&BoLHUsers=${BoLHUsers}`
      ...
      render: (text, record) => (
        <div>
          <Paragraph className="my-0 text-md font-medium">
            <Link href={`/customer/${record._id}`}>
              <a href={`/customer/${record._id}`}>{record.name || record.rfc}</a>
            </Link>
          </Paragraph>
  ...
  const dataForTable = data.data.customersData.map(customer => {
      ...
      <Layout>
        ...
        <Layout.Content className="p-4">
          <CustomersPageHeader
            ...
            BoLHClients={BoLHClients}
          />
          <CustomersList dateRange={dateRange} BoLHClients={BoLHClients} BoLHUsers={BoLHUsers} />
        </Layout.Content>
      </Layout>
